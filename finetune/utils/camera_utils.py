...
@torch.no_grad()
@torch.autocast(device_type="cuda", enabled=False)
def get_camera_condition(camera_intrinsics, camera_extrinsics, cond_frame_index, trace_scale_factor, per_frame_scale, H, W, *args, **kwargs):
    '''
    ...
    if per_frame_scale is not None:
        depth_scale = per_frame_scale[torch.arange(B, device=device), cond_frame_index]
